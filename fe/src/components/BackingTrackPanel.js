...
import { FaMusic, FaPlay, FaPlus, FaMagic, FaTrash, FaCheck, FaTimes } from "react-icons/fa";
...
const BackingTrackPanel = ({
  ...
  const selectedRhythm = rhythmPatterns.find(r => r._id === selectedRhythmPatternId);
  ...
  const canGenerate = chordProgression.length > 0 && selectedInstrumentId && selectedRhythmPatternId;

  return (
    <div className="h-full flex flex-col bg-gray-900 text-white overflow-y-auto">
      {/* Header with Steps Indicator */}
      <div className="p-3 border-b border-gray-700 bg-gray-800/50">
        <h3 className="text-sm font-semibold text-white mb-2">Create Backing Track</h3>
        <div className="flex items-center gap-2 text-xs">
          <div className={`flex items-center gap-1 ${chordProgression.length > 0 ? 'text-green-400' : 'text-gray-500'}`}>
            <div className={`w-4 h-4 rounded-full flex items-center justify-center ${chordProgression.length > 0 ? 'bg-green-500' : 'bg-gray-700'}`}>
              {chordProgression.length > 0 && <FaCheck size={8} />}
            </div>
            <span>1. Chords</span>
          </div>
          <div className="w-4 h-0.5 bg-gray-700"></div>
          <div className={`flex items-center gap-1 ${selectedInstrumentId ? 'text-green-400' : 'text-gray-500'}`}>
            <div className={`w-4 h-4 rounded-full flex items-center justify-center ${selectedInstrumentId ? 'bg-green-500' : 'bg-gray-700'}`}>
              {selectedInstrumentId && <FaCheck size={8} />}
            </div>
            <span>2. Instrument</span>
          </div>
          <div className="w-4 h-0.5 bg-gray-700"></div>
          <div className={`flex items-center gap-1 ${selectedRhythmPatternId ? 'text-green-400' : 'text-gray-500'}`}>
            <div className={`w-4 h-4 rounded-full flex items-center justify-center ${selectedRhythmPatternId ? 'bg-green-500' : 'bg-gray-700'}`}>
      ...
      <div className="p-3 border-b border-gray-700 flex-shrink-0">
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs font-semibold text-gray-300">
            Chord Progression {chordProgression.length > 0 && `(${chordProgression.length})`}
          </label>
        ...
        {chordProgression.length === 0 ? (
          <div className="text-center py-4 border-2 border-dashed border-gray-700 rounded-lg">
            ...
            <p className="text-[10px] text-gray-600">Click chords from the library to add them</p>
          </div>
        ) : (
          <div className="flex flex-wrap gap-1.5">
            {chordProgression.map((chord, index) => (
              <div
                key={index}
                className="group relative px-3 py-1.5 bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-lg text-xs font-semibold flex items-center gap-2 hover:from-indigo-500 hover:to-indigo-600 transition-all shadow-sm"
              >
                <span className="text-[10px] text-indigo-200 font-normal">{index + 1}</span>
                <span>{chord?.chordName || chord?.name || `Chord ${index + 1}`}</span>
                {onRemoveChord && (
                  <button
      ...
      <div className="p-3 border-b border-gray-700 flex-shrink-0">
        ...
        {instruments.length === 0 ? (
          <div className="text-xs text-gray-500 py-2">Loading instruments...</div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex flex-col items-center gap-1">
                    <FaMusic size={14} className={isSelected ? "text-orange-200" : "text-gray-500"} />
                    <span className="truncate w-full text-center text-[10px]">{instrument.name}</span>
                  </div>
                </button>
              );
            })}
        ...
        {selectedInstrument && (
          <div className="mt-2 px-2 py-1.5 bg-orange-900/30 border border-orange-700/50 rounded text-xs text-orange-200">
            <span className="font-medium">Selected:</span> {selectedInstrument.name}
          </div>
        )}
      </div>
      ...
      <div className="p-3 border-b border-gray-700 flex-shrink-0 space-y-2">
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            <label className="block text-xs font-medium mb-1 text-gray-400">AI Style</label>
            <select
              value={aiStyle}
      ...
      <div className="p-3 space-y-2 flex-shrink-0">
        ...
        {canGenerate && (
          <div className="text-center px-2 py-1.5 bg-green-900/30 border border-green-700/50 rounded text-xs text-green-300">
            ✓ Ready to generate {chordProgression.length} chord{chordProgression.length !== 1 ? 's' : ''} with {selectedInstrument?.name || 'instrument'}
          </div>
        )}
      </div>
    </div>
